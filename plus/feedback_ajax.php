...
<DIV id=comment-10827210 class="comment-block floor-1">
<DIV class=clearfix>
<DIV class=avatar><IMG src="<?php echo $mface;?>"> </DIV><!-- avatar-->
<DIV class=reply><SPAN class=anomynous><?php echo $username; ?></SPAN> <SPAN class=body><?php echo ubb($msg); ?></SPAN></DIV>
<div class="report">
<span>new</span>
</div>
</DIV></DIV>
...
function GetList($page=1)
{
    ...
    $querystring = "SELECT fb.*,mb.userid,mb.face as mface,mb.spacesta,mb.scores,mb.sex FROM `#@__feedback` fb
                 LEFT JOIN `#@__member` mb on mb.mid = fb.mid WHERE fb.aid='$aid' AND fb.ischeck='1' ORDER BY fb.id ASC";
    ...
    //    return $totalcount;
    //}
    $dsql->Execute('fb', $querystring." LIMIT 120 ");
